...
w.dir <- "Y:/mbh-d3-swc-fish"
p.dir <- paste(m.dir, "plots", sep = '/')
d.dir <- paste(m.dir, "data", sep='/')
s.dir <- paste(m.dir, "shapefiles", sep='/')
r.dir <- paste(m.dir, "rasters", sep='/')


### Read sw cmr polys ----
cmr <- readOGR(paste(s.dir, "AustraliaNetworkMarineParks.shp",sep='/'))
plot(cmr)
names(cmr)
head(cmr)
...
## Read Bathy ----
bathy <- raster(paste(r.dir, "Bathy.tif", sep='/'))
#bathy <- raster(paste(r.dir, "WA_500m_Bathy.tif", sep='/'))
plot(bathy)

#values(bathy)[values(bathy) < -2500] = NA
#plot(bathy)
plot(cmr, add=T)
...
# e <- drawExtent()
# e
ext1 <- extent(113.1145   , 114.4722, -23.40797 , -21.48672  )
...
map7 <- map55 + tm_shape(MUZ) + tm_borders(col = 'gray', lwd = 1.5) + 
...
map8 <- map7 + tm_shape(wamp1) + tm_borders(col = 'black', lwd = 1.5, lty ='33') + tm_fill(col='pink', alpha = 0.3) +
...
map <- tm_shape(cmr1, bbox=ext1)  + tm_borders(col ='white', lwd = 1.5) +
  ...
  #tm_fill(col ='ZONENAME', palette=c('yellow', 'red'), alpha = 0.1) +
  tm_scale_bar(breaks = c(0, 25, 50), text.size = 0.5, position = c(0.2, 0.02))  
#tm_graticules(ticks = FALSE) +
#tm_grid(n.x = 3, n.y = 3, labels.size = 1.5, lines = FALSE) 
map
...
map7 <- map55 + tm_shape(NPZ) + tm_borders(col = "#006600", lwd = 2) 
...
map8 <- map71 + tm_shape(wamp1) + tm_borders(col = "#006600", lwd = 2)  #+ tm_fill(col='pink', alpha = 0.4) +
...
tmap_save(
  tm = map9,
  filename = paste(p.dir, "Ningaloo-Sea-level.png", sep ='/'),
  #width = 2000,
  #height = 2200,
  #units = NA,
  scale = 1,
...
tmap_save(
  tm = map.leg,
  filename = paste(p.dir, "Ningaloo-Sea-level-legend.png", sep ='/'),
  #width = 2000,
  #height = 2200,
  #units = NA,
  scale = 1,
